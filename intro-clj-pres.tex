...
\begin{frame}{Teasers}
	\begin{enumerate}[<+->]
	\item Add sum of squares of all numbers in a list
...
\begin{frame}{Teaser \#1}
	\begin{itemize}
        \item Idea: Add sum of squares of all numbers in a list
        \item Java\\
\begin{small}
{\ttfamily\color{black}
%
...
\begin{frame}{Closures}
  ...
  \item \texttt{soln-fn} is a closure -- the values of
    \texttt{a} and \texttt{b} are pulled from surrounding scope
...
\#\{1 2 3 12\}}
  \item \texttt{assoc} (for maps) adds a key and its value, \texttt{dissoc}
    removes a key and value given a key\\
...
{\ttfamily\color{black}
[3 8 9 5 -1 4 1 6]}
  \item When what you want comes as a sequence:\\
{\ttfamily\color{black}
\textcolor[rgb]{0.49803922,0.0,0.49803922}{user{\textgreater}
...
{\ttfamily\color{black}
(3 9 5 -1 1)}
  \item Use \texttt{apply} to ``unpack'' the sequence and apply the
    fn:\\
...
\begin{frame}[allowframebreaks]{Macros}
  ...
  \item Only possible when language's code written in language's data
    ...
    \framebreak
  \item Threading macros (\texttt{->} and \texttt{->>})
    \begin{itemize}
...
{\textbackslash}f\}\textcolor[rgb]{0.54901963,0.54901963,0.54901963}{)))}}
  ...
  \item Macros allow individuals to add on to their language
    ...
    \item \texttt{with-open}
      ...
      \item ``Imported'' into Python, but can only be done as language
        syntax (only by language maintainers)
      \end{itemize}
    \item The \texttt{some->} macro
      \begin{itemize}
    ...
    \item Most of Clojure is implemented as fns and macros
      \begin{itemize}
      \item A few ``special form'' fns exist as elemental building
        blocks
      \item Rest of language is composed of previously-defined fns and
        macros
...
\begin{frame}{High-level Design Decision Cascade}
  ...
  \item Functional programming $\rightarrow$ referential transparency
  \end{itemize}
\end{frame}
...
\begin{frame}{My Parting Message to You}
  ...
  \item Clojure/Lisp compared to other languages
    ...
    \item But maybe a powerful vs. more powerful
      \begin{itemize}
    ...
    \item Tradeoffs exist -- always choose right tool for the job
      ...
      \end{itemize}
    \item Language discussions $\rightarrow$ emotional arguments b/c of proximity
      to mind \& identity
      \begin{itemize}
...
\begin{frame}{Abridged Set of Useful Resources}
  ...
  \item Videos of Easy-to-follow Lectures by Rich Hickey
    \begin{itemize}
    \item Data structures (Pt. 1 \& Pt. 2); Sequences; Concurrency
    \item Clojure for Java Programmers (Pt. 1 \& Pt. 2)
    \item Clojure for Lisp Programmers (Pt. 1 \& Pt. 2)
    \end{itemize}
